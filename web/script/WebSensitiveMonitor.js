WebSensitiveMonitor=function(){
	this.init=function(config){
		var store= new Ext.data.JsonStore({
			url:'PHP/WebWordSensitive.php',
			root:'data',
			totalProperty:'total',
			fields:[{ name: 'id'},
					{ name: 'word', type: 'char'}
		...
		var WebData=new Ext.data.JsonStore({
			...
			fields:[
			     ...
			     {name: 'sensitive_words',type: 'string'},
				 {name: 'source', type: 'string'}
				 ]
		});
		WebData.load({	//当点击该主题时自动加载所有监视内容
			params:{
				start:0,
				limit:30	
			}
...
autoExpandColumn: 	'title',
			...
			tbar:[
				{ text: '敏感词' },
				storeList,
				{xtype: 'tbspacer', width: 50}, 	//引入间格
				{
					xtype: 'button',	//！？？？？？怎么实现得到用户选的敏感词，并形成Mysql查询语句？？？？？？--------------
					text:'提 交',
					width: 100,
					//columnWidth: .10,	
					loadMask: true, 	//显示 数据读取中...
				 	listeners:{
						scope:this,
...
//测试实现表格的自动伸缩功能
			columns: [new Ext.grid.RowNumberer({width: 20}),
					...
					{ header: '敏感词',dataIndex: 'sensitive_words',width: 100,sortable:true },
					{ header: '发布站点', dataIndex: 'source', width: 50, sortable: true }					
				]
		/*	cm: new Ext.grid.ColumnModel({
				...
				columns: [
					...
					{ header: '敏感词',dataIndex: 'sensitive_words',width: 150,sortable:true },
