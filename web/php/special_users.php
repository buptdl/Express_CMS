...
//连接到数据库
	...
	if (isset($_POST['filters']))
		$filter="where '".$_POST['filters']['0']['data']['value']."'=board";
	else
		$filter='';
	$query="SELECT distinct user AS author,publish_num,publish_floor,reply_num,reply_floor,emotion_tendy,activeness,date(FROM_UNIXTIME(date)) AS publish_date FROM pris_users_special ".$filter.' ORDER BY date DESC,activeness desc limit 0,25';
	//echo $query;
